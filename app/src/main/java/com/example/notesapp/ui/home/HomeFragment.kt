...
//        homeViewModel.deleteAllData()
    ...
    private fun confirmDelete() {
        if (currentData.isEmpty()) {
            ...
            AlertDialog.Builder(requireContext())
                ...
                .setPositiveButton("InsyaaAlloh") { _, _ ->
                    homeViewModel.deleteAllData()
                    Toast.makeText(
                        requireContext(),
                        "Yeayy Berhasil Hapus data",
                        Toast.LENGTH_SHORT
                    ).show()
                }
                .setNegativeButton("GAK") { dialog, _ ->
                    dialog.dismiss()
                }.show()
        }
    }
    ...
    private fun swipeToDelete(recyclerView: RecyclerView) {
        val swipeToDelete = object : ItemTouchHelper.SimpleCallback(// ItemTouchHelper supaya bisa menggeser kekiri atau kekanan
            ...
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
                ...
                restoredDate(viewHolder.itemView, deletedItem)
            }
        }
    ...
    private fun restoredDate(view: View, deletedItem: Notes) {
        val snackbar = Snackbar.make(
            view, "Note Deleted ${deletedItem.title}", Snackbar.LENGTH_LONG
        )
