...
class DetailFragment : Fragment() {
    ...
    private val saveArgs by navArgs<DetailFragmentArgs>()
            ...
            R.id.menu_edit ->{
                val action = DetailFragmentDirections.actionDetailFragmentToUpdateFragment(
                    Notes(
                        ...
                        saveArgs.currentItem.title,
                        saveArgs.currentItem.priority,
                        saveArgs.currentItem.decs,
                        saveArgs.currentItem.date
                    )
                )
    ...
    private fun confirmDeleteNote() {
        context?.let {
            AlertDialog.Builder(it).setTitle("Delete '${saveArgs.currentItem.title}'?")
                .setMessage("If you sure want to remove '${saveArgs.currentItem.title}'. it will be delete for permanent")
                .setPositiveButton("of course") { _, _ ->
                    detailViewModel.deleteNote(saveArgs.currentItem)
